function[] = matipp_fmri_prepare(source_dir, log_dir, output_4d_dir, output_3d_dir, output_type, pipeline_opt)
...
setenv('FSLOUTPUTTYPE',output_type);
...
%        output_image = [subj_3d_dir, filesep, name, '_',num2str(j),output_type];
        output_image = [subj_3d_dir, filesep, name, '_',num2str(j)];
        job_name{job_id} = ['fslsplit_',subj_id,'_',num2str(j)];
        pipeline.(job_name{job_id}).command           = 'fsl_split(files_in, files_out, opt.parameters1, opt.parameters2, opt.parameters3)';
        pipeline.(job_name{job_id}).files_in          = pipeline.(job_name{1}).files_out;
        pipeline.(job_name{job_id}).files_out         = output_image;
        pipeline.(job_name{job_id}).opt.parameters1   = j - 1;
        pipeline.(job_name{job_id}).opt.parameters2   = 1;
        pipeline.(job_name{job_id}).opt.parameters3   = output_type;
        job_id = job_id + 1;
    end
