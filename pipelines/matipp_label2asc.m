function[] = matipp_label2asc(subjects_dir, log_dir, pipeline_opt)
% Pipeline for convert freesurfer label file to ascii surface file.
%
%--------------------------------------------------------------------------
%     wanlin zhu
...
for i = 1 : nums
    ...
    for j = 1 : 2
        ...
        %3. label2asc
        job_name{3} = ['label2asc_',subj_id,'_',hemi{j}];
        pipeline.(job_name{3}).command           = 'mkdir(files_out); label2asc(files_in{2}, files_out, files_in{1})';
